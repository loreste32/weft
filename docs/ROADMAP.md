...
The **0.3.x line is complete** (0.3.31–0.4.7). Everything shipped.
...
**Shipped in 0.4.x so far (0.4.0–0.4.7):** optional type annotations + `--strict`, DAP debugging, browser Wasm playground, registry namespace trust, telecom SIP REFER / WebRTC bridge, VS Code 0.4.7 (LSP types + DAP), bytecode validation, fuzz/race/bench smoke targets, grouped imports, registry auto-fetch, third-party git imports, LSP references/rename/extract/auto-import, REPL tab completion + multi-line polish, compat corpus expansion, glue benchmarks vs Python, reference apps (`ref_agent_ops`, `ref_http_glue`, `ref_ops`), tag-triggered release workflow.
...
- VS Code extension 0.4.7 VSIX packaged; Marketplace publish needs `VSCE_PAT`
